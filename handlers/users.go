...
func GetUsers(c *gin.Context) {
	...
	if contentType == "application/xml" || contentType == "text/xml" {
		...
		respondWithData(c, http.StatusOK, users)
	}
}
...
func GetUserPreferences(c *gin.Context) {
	...
	for _, user := range users {
		if user.ID == id {
			respondWithData(c, http.StatusOK, user.Preferences)
			return
		}
	}
...
func UpdateUserPreferences(c *gin.Context) {
	...
	for i, user := range users {
		if user.ID == id {
			...
			users[i].UpdatedAt = time.Now()
			respondWithData(c, http.StatusOK, users[i].Preferences)
			return
		}
	}
...
func GetUserProfile(c *gin.Context) {
	...
	for _, user := range users {
		if user.ID == id {
			respondWithData(c, http.StatusOK, user.Profile)
			return
		}
	}
...
func UpdateUserProfile(c *gin.Context) {
	...
	for i, user := range users {
		if user.ID == id {
			users[i].Profile = req
			users[i].UpdatedAt = time.Now()
			respondWithData(c, http.StatusOK, users[i].Profile)
			return
		}
	}
...
// ActionListUsers godoc
// @Summary List all users via action parameter
// @Description Get a list of all users using action=list parameter
// @Tags user-actions
// @Accept json,xml
// @Produce json,xml
// @Param action query string true "Action to perform" Enums(list)
// @Success 200 {array} models.User
// @Success 200 {object} models.UserListResponse
func ActionListUsers(c *gin.Context) {
	action := c.Query("action")
	if action != "list" {
		respondWithError(c, http.StatusBadRequest, "Invalid action. Use action=list")
		return
	}
...
// ActionGetUser godoc
// @Summary Get a user by ID via action parameter
// @Description Get a single user by their ID using action=get parameter
// @Tags user-actions
// @Accept json,xml
// @Produce json,xml
// @Param action query string true "Action to perform" Enums(get)
// @Param id query int true "User ID"
// @Success 200 {object} models.User
// @Failure 400 {object} models.ErrorResponse
// @Failure 404 {object} models.ErrorResponse
func ActionGetUser(c *gin.Context) {
	action := c.Query("action")
	if action != "get" {
		respondWithError(c, http.StatusBadRequest, "Invalid action. Use action=get")
		return
	}
...
// ActionCreateUser godoc
// @Summary Create a new user via action parameter
// @Description Create a new user with the provided information using action=create parameter
// @Tags user-actions
// @Accept json,xml
// @Produce json,xml
// @Param action query string true "Action to perform" Enums(create)
// @Param user body models.CreateUserRequest true "User information with nested structures"
// @Success 201 {object} models.User
// @Failure 400 {object} models.ErrorResponse
func ActionCreateUser(c *gin.Context) {
	action := c.Query("action")
	if action != "create" {
		respondWithError(c, http.StatusBadRequest, "Invalid action. Use action=create")
		return
	}
...
// ActionUpdateUser godoc
// @Summary Update a user via action parameter
// @Description Update an existing user with the provided information using action=update parameter
// @Tags user-actions
// @Accept json,xml
// @Produce json,xml
// @Param action query string true "Action to perform" Enums(update)
// @Param id query int true "User ID"
// @Param user body models.UpdateUserRequest true "Updated user information with nested structures"
// @Success 200 {object} models.User
// @Failure 400 {object} models.ErrorResponse
// @Failure 404 {object} models.ErrorResponse
func ActionUpdateUser(c *gin.Context) {
	action := c.Query("action")
	if action != "update" {
		respondWithError(c, http.StatusBadRequest, "Invalid action. Use action=update")
		return
	}
...
// ActionDeleteUser godoc
// @Summary Delete a user via action parameter
// @Description Delete a user by their ID using action=delete parameter
// @Tags user-actions
// @Accept json,xml
// @Produce json,xml
// @Param action query string true "Action to perform" Enums(delete)
// @Param id query int true "User ID"
// @Success 200 {object} models.SuccessResponse
// @Failure 400 {object} models.ErrorResponse
// @Failure 404 {object} models.ErrorResponse
func ActionDeleteUser(c *gin.Context) {
	action := c.Query("action")
	if action != "delete" {
		respondWithError(c, http.StatusBadRequest, "Invalid action. Use action=delete")
		return
	}
...
// ActionSearchUsers godoc
// @Summary Search users by name or email via action parameter
// @Description Search for users by name or email using action=search parameter
// @Tags user-actions
// @Accept json,xml
// @Produce json,xml
// @Param action query string true "Action to perform" Enums(search)
// @Param query query string true "Search query (name or email)"
// @Success 200 {array} models.User
// @Success 200 {object} models.UserListResponse
// @Failure 400 {object} models.ErrorResponse
func ActionSearchUsers(c *gin.Context) {
	action := c.Query("action")
	if action != "search" {
		respondWithError(c, http.StatusBadRequest, "Invalid action. Use action=search")
		return
	}

	searchQuery := c.Query("query")
	if searchQuery == "" {
		respondWithError(c, http.StatusBadRequest, "Search query is required")
		return
	}
	...
	for _, user := range users {
		...
		if strings.Contains(strings.ToLower(user.Name), strings.ToLower(searchQuery)) ||
			strings.Contains(strings.ToLower(user.Email), strings.ToLower(searchQuery)) {
	...
	contentType := c.GetHeader("Accept")
	if contentType == "application/xml" || contentType == "text/xml" {
		response := models.UserListResponse{Users: matchedUsers}
		c.XML(http.StatusOK, response)
	} else {
...
// @Accept json,xml
// @Produce json,xml
// @Param action query string true "Action to perform" Enums(list,get,search,create,update,delete)
// @Param id query int false "User ID (required for get, update, delete actions)"
// @Param user body models.User false "User object (required for create and update actions)"
// @Success 200 {object} models.User
// @Success 200 {array} models.User
// @Success 200 {object} models.UserListResponse
// @Success 201 {object} models.User
// @Failure 400 {object} models.ErrorResponse
// @Failure 404 {object} models.ErrorResponse
// @Router /user-action [get]
// @Router /user-action [post]
// @Router /user-action [put]
// @Router /user-action [delete]
func UserActionHandler(c *gin.Context) {
	action := c.Query("action")
	if action == "" {
		respondWithError(c, http.StatusBadRequest, "Action parameter is required")
		return
	}
	...
	case "GET":
		...
		case "search":
			ActionSearchUsers(c)
		default:
			respondWithError(c, http.StatusBadRequest, "Invalid action for GET method. Use 'list', 'get', or 'search'")
		}
	case "POST":
		switch action {
		case "create":
			ActionCreateUser(c)
		default:
			respondWithError(c, http.StatusBadRequest, "Invalid action for POST method. Use 'create'")
		}
	case "PUT":
		switch action {
		case "update":
			ActionUpdateUser(c)
		default:
			respondWithError(c, http.StatusBadRequest, "Invalid action for PUT method. Use 'update'")
		}
	case "DELETE":
		switch action {
		case "delete":
			ActionDeleteUser(c)
		default:
			respondWithError(c, http.StatusBadRequest, "Invalid action for DELETE method. Use 'delete'")
		}
	default:
